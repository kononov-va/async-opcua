...
pub(crate) fn request_lite_m_volume(device_lite_m: &Device, time_stamp: TimestampsToReturn) -> Result<data_value::DataValue, io::Error> {
...
pub(crate) fn request_lite_m_arhive_period(device: &Device, start: opcua::types::data_types::UtcTime,
    ...
    if !bounds && num_values_per_node == 1 {
        if start == opcua_types::DateTime::epoch() && end == opcua_types::DateTime::endtimes() {
            //last point
            let (result, status) = request_lite_m_end_point(device, time_stamp);
            return (result, status, None);
        }
        else if start == opcua_types::DateTime::epoch() + chrono::TimeDelta::seconds(1) && end == opcua_types::DateTime::epoch() {
            //oldest point
            let (result, status) = request_lite_m_start_point(device, time_stamp);
            return (result, status, None);
        }
    }
    
    let (period, continuation) = crate::period_util::get_period(start, end, bounds, num_values_per_node);
    ...
    match period {
        Some(period) => {
            ...
            (Some(result), status_result, continuation)
        },
        None => (None, status_result, None),
    }
}
...
fn read_arc_answer(session_id: u16, device_lite_m: &Device, mut str: TcpStream, time_stamp: TimestampsToReturn) -> Result<data_value::DataValue, io::Error> {
    ...
    match str.read(&mut answer) {
        Ok(_) => {
            let id = u16::from_be_bytes([answer[0], answer[1]]);
            if id == session_id{
                let mut status_code = opcua_types::StatusCode::Good;
                let time_source = u32::from_be_bytes([answer[9], answer[10], answer[11], answer[12]]);
                if time_source == 0 {
                    status_code = opcua_types::StatusCode::BadNoData;
                }
