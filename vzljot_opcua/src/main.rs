...
use opcua::{
    types::{data_value, node_id, TimestampsToReturn},//DateTime, 
...
fn get_device_current_volume(device: &Device, time_stamp: TimestampsToReturn) -> Result<data_value::DataValue, io::Error> {
    match device.device_type {
        DeviceType::LiteM => {
...
fn request_lite_m_arch_index(device_lite_m: &Device, time_stamp: TimestampsToReturn) -> Result<u16, io::Error> {
...
fn request_lite_m_end_point(device: &Device, time_stamp: TimestampsToReturn) -> (Option<Vec<opcua::types::data_value::DataValue>>, opcua_types::StatusCode) {
    ...
    let index = match request_lite_m_arch_index(device, time_stamp) {
        Ok(i) => i,
...
fn request_lite_m_start_point(device: &Device, time_stamp: TimestampsToReturn) -> (Option<Vec<opcua::types::data_value::DataValue>>, opcua_types::StatusCode) {
    ...
    let mut index = match request_lite_m_arch_index(device, time_stamp) {
        Ok(i) => i,
        Err(_) => {return (None, status_result);}
    };
    index += 1;
